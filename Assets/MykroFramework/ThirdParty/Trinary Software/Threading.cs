...
// /////////////////////////////////////////////////////////////////////////////////////////
//                              More Effective Coroutines Threaded
//                                        v3.08.0
// 
// The module works with MEC (both free and pro) to add efficient multithreading in Unity using
// standard MEC coroutines.
// 
    ...
    public class Threading : MonoBehaviour
    {
        ...
        private static Threading Instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    _instance = instanceHome.GetComponent<Threading>() ?? instanceHome.AddComponent<Threading>();
        ...
        private void Initialize()
        {
			if (!initialized) 
			{
				...
				Timing.OnPreExecute += ReturnAllProcesses;
        ...
        public static float SwitchToExternalThread(UnityEngine.ThreadPriority priority)
        {
            ...
            Timing.ReplacementFunction = RetrieveCoroutine;
        ...
        private static IEnumerator<float> RetrieveCoroutine(IEnumerator<float> coptr, CoroutineHandle handle) 
        {
            ...
            Timing.GetInstance(handle.Key).LockCoroutine(handle, Instance._coroutineKey);
            return coptr;
        }
        ...
        private void ReturnProcess(Process process)
        {
            ...
            {
                Timing.KillCoroutines(process.Handle);
                _exceptions.Enqueue(process.Exception);
            }
            else
            {
                Timing.GetInstance(process.Handle.Key).UnlockCoroutine(process.Handle, _coroutineKey);
            }
         }
        ...
        {
            if(Thread.CurrentThread == Timing.MainThread)
